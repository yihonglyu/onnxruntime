...
MLAS_FORCEINLINE
int32_t
MlasGemmU8X8FixupZeroPointA<MLAS_GEMM_S8S8_KERNEL_NEON>(
    int32_t ZeroPointA
    )
{
...
//printf("doit%s %zd %zd %zd (packed=%d,signed=%d) %d\n", USE_S8S8 ? "s8s8" : "u8u8", M, N, K, DataParams->BIsPacked, Shape.BIsSigned, int(t1-t0));
}
