...
void PackGemmAll(benchmark::State& state, bool b_is_signed, bool pack) {
  ...
  for (size_t i = 0; i < batch; i++) {
    ...
    gemm_params.PackedB = (void*)(AlignAddr(pack_b_holder.data()) + packed_b_size * i);
  ...
  for (auto _ : state) {
    if (pack) {
      for (size_t i = 0; i < batch; i++) {
        ...
        MlasGemmPackAandB(M, N, K, AlignAddr(A_holder.data()) + M * K * i, gemm_params.lda, gemm_params.PackedA, AlignAddr(B_holder.data()) + N * K * i, b_is_signed, gemm_params.ldb, gemm_params.PackedB, tp.get());
