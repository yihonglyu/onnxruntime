...
static void ComputeGemm(
    ...
    size_t K,
    size_t N,
    const uint8_t* a_data, uint8_t a_zero_point,
    const uint8_t* b_data, uint8_t b_zero_point,
    bool b_is_signed,
    int32_t* y_data,
...
static void ComputePackGemm(
    ...
    int32_t* y_data,
    onnxruntime::concurrency::ThreadPool* tp) {
  MlasGemmPackAandB(M, N, K, a_data, K, a_packed, b_data, b_is_signed, N, b_packed, tp);
...
void PackGemmAll(benchmark::State& state, bool b_is_signed, bool quant_pack) {
  ...
  for (auto _ : state) {
    if (quant_pack) {
      ComputePackGemm(M, K, N, AlignAddr(A_holder.data()), a_zero_point, AlignAddr(pack_a_holder.data()),
                      AlignAddr(B_holder.data()), b_zero_point, AlignAddr(pack_b_holder.data()),
                      b_is_signed, AlignAddr(C_holder.data()), tp.get());
    } else {
      ComputeGemm(M, K, N, AlignAddr(A_holder.data()), a_zero_point,
                  AlignAddr(B_holder.data()), b_zero_point, b_is_signed,
...
BENCHMARK_CAPTURE(PackGemmAll, SignedB, true, false)->Apply(QGemmSize)->UseRealTime();
BENCHMARK_CAPTURE(PackGemmAll, SignedBPack, true, true)->Apply(QGemmSize)->UseRealTime();
BENCHMARK_CAPTURE(PackGemmAll, UnSignedB, false, false)->Apply(QGemmSize)->UseRealTime();
BENCHMARK_CAPTURE(PackGemmAll, UnSignedBPack, false, true)->Apply(QGemmSize)->UseRealTime();
