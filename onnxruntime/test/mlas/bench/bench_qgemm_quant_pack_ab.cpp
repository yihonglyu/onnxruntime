...
//
// Comparing
// Quantize A + Quantize B + QGemm
//  vs
// Quantize-pack A + Quantize-pack A + QGemm
//
...
static void ComputeQuantizeGemm(
    ...
    const float* b_data, float b_scale, uint8_t b_zero_point,
    bool b_is_signed,
    uint8_t* b_data_quant,
    int32_t* y_data,
    onnxruntime::concurrency::ThreadPool* tp) {
  onnxruntime::ParQuantizeLinear(a_data, a_data_quant, M * K, a_scale, a_zero_point, tp);
  if (b_is_signed) {
    onnxruntime::ParQuantizeLinear<int8_t>(b_data, (int8_t*)(b_data_quant), N * K, b_scale, b_zero_point, tp);
  } else {
    onnxruntime::ParQuantizeLinear<uint8_t>(b_data, b_data_quant, N * K, b_scale, b_zero_point, tp);
  }
  ...
  gemm_data.BIsPacked = false;
  gemm_data.B = b_data_quant;
...
static void ComputeQuantizePackGemm(
    ...
    const float* b_data, float b_scale, uint8_t b_zero_point,
    uint8_t* b_data_quant_packed,
    bool b_is_signed,
    int32_t* y_data,
    onnxruntime::concurrency::ThreadPool* tp) {
  MlasQuantizeLinearPackAandB(M, N, K, a_data, K, a_scale, a_zero_point, a_data_quant_packed,
                              b_data, b_is_signed, N, b_scale, b_zero_point, b_data_quant_packed, tp);
  ...
  gemm_data.B = b_data_quant_packed;
  gemm_data.ldb = gemm_shape.N;
  gemm_data.ZeroPointB = &b_zero_point;
  gemm_data.C = y_data;
  gemm_data.ldc = gemm_shape.N;
...
void QuantPackGemmAll(benchmark::State& state, bool b_is_signed, bool quant_pack) {
  ...
  auto A_holder = RandomVectorUniform<float>(static_cast<size_t>(M * K * batch) + MLAS_BENCH_BUF_ALIGN, -15.0f, 20.0f);
  float a_scale;
  uint8_t a_zero_point;
  onnxruntime::GetQuantizationParameter(AlignAddr(A_holder.data()), M * K * batch, a_scale, a_zero_point, tp.get());

  auto B_holder = RandomVectorUniform<float>(static_cast<size_t>(N * K * batch) + MLAS_BENCH_BUF_ALIGN, -25.0f, 79.0f);
  float b_scale;
  uint8_t b_zero_point;
  onnxruntime::GetQuantizationParameter(AlignAddr(B_holder.data()), N * K * batch, b_scale, b_zero_point, tp.get());
  ...
  for (auto _ : state) {
    if (quant_pack) {
      ComputeQuantizePackGemm(M, K, N, AlignAddr(A_holder.data()), a_scale, a_zero_point,
                              ...
                              AlignAddr(B_holder.data()), b_scale, b_zero_point,
                              AlignAddr(pack_b_holder.data()),
                              b_is_signed, AlignAddr(C_holder.data()), tp.get());
    } else {
      ComputeQuantizeGemm(M, K, N, AlignAddr(A_holder.data()), a_scale, a_zero_point,
                          AlignAddr(pack_a_holder.data()),
                          AlignAddr(B_holder.data()), b_scale, b_zero_point, b_is_signed,
                          AlignAddr(pack_b_holder.data()),
...
BENCHMARK_CAPTURE(QuantPackGemmAll, SignedB, true, false)->Apply(QGemmSize)->UseRealTime();
BENCHMARK_CAPTURE(QuantPackGemmAll, SignedBPack, true, true)->Apply(QGemmSize)->UseRealTime();
BENCHMARK_CAPTURE(QuantPackGemmAll, UnSignedB, false, false)->Apply(QGemmSize)->UseRealTime();
BENCHMARK_CAPTURE(QuantPackGemmAll, UnSignedBPack, false, true)->Apply(QGemmSize)->UseRealTime();
