...
class QgemmShortExecuteTest<AType, BType, int32_t, Packed, Threaded> : public MlasTestFixture<MlasQgemmTest<AType, BType, int32_t, Packed, Threaded>> {
 public:
  ...
  static size_t RegisterShortExecuteTests() {
    ...
    size_t dims[] = {16, 32, 48, 64, 80, 96, 400, 1024};
    size_t kdims[] = {41, 59, 71, 384 + 20, 1003};
    for (size_t m : dims){
      for (size_t n : dims){
        for (size_t k : kdims){
          test_registered += RegisterSingleTest(m, n, k, 1, 14, 211);
          test_registered += RegisterSingleTest(m, n, k, 1, 17);
        }
      }
    }
    test_registered += RegisterSingleTest(1024, 2048, 4096, 1, 21);
